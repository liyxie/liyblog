...
/** 重置表单 */
function resetForm() {
  console.log(jobFormRef.value);
  if (jobFormRef.value != null) {
...
<template>
  <div class="app-container">
    <div class="search-container">
      <el-form ref="queryFormRef" :model="queryParams" :inline="true">
        ...
        <el-form-item label="任务组名" prop="jobGroup">
          <el-select @change="handleQuery" v-model="queryParams.jobGroup" style="width: 200px" placeholder="请选择任务组名"
            clearable>
            <el-option v-for="dict in jobDictList" :key="dict.value" :label="dict.label" :value="dict.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="任务状态" prop="status">
          <el-select @change="handleQuery" v-model="queryParams.status" style="width: 200px" placeholder="请选择任务状态"
            clearable>
    ...
    <el-card shadow="never" class="table-container">
      <template #header>
        <el-button type="success" @click="openDialog()" v-hasPerm="['system:job:add']"><i-ep-plus />新增</el-button>
        <el-button type="danger" :disabled="ids.length === 0" @click="handleDelete"
          v-hasPerm="['system:job:delete']"><i-ep-delete />批量删除</el-button>
        <el-button type="info" icon="Operation" @click="handleJobLog">日志</el-button>
      </template>
      ...
      <el-table ref="dataTableRef" v-loading="loading" :data="tabelData" highlight-current-row stripe fit
        ...
        <el-table-column label="任务组名" align="center">
          <template #default="scope">
            <span v-for="(dict, index) in jobDictList" :key="index">
              <el-tag v-if="dict.value == scope.row.jobGroup" :type="dict.style">
                {{ dict.label }}
              </el-tag>
            </span>
          </template>
        ...
        <el-table-column label="状态" align="center">
          <template #default="scope">
            <el-switch v-model="scope.row.status" active-value="0" inactive-value="1" v-hasPerm="['system:job:update']"
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-button type="primary" link @click="openDialog(scope.row)" size="small" icon="Edit"
              v-hasPerm="['system:job:update']">修改</el-button>
            <el-button type="danger" link @click="handleDelete(scope.row.jobId)" size="small" icon="Delete"
              v-hasPerm="['system:job:delete']">删除</el-button>
            <el-dropdown @command="(command) => handleCommand(command, scope.row)" size="small">
              <el-button type="primary" style="margin-left: 9px" link>更多
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item command="handleRun" icon="CaretRight"
                    v-hasPerm="['system:job:run']">执行一次</el-dropdown-item>
                  <el-dropdown-item command="handleView" icon="View">任务详细</el-dropdown-item>
                  <el-dropdown-item command="handleJobLog" icon="Document">调度日志</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
          </template>
            ...
            <el-col :span="24">
              <el-form-item label="错误策略" prop="misfirePolicy">
                <el-radio-group v-model="formData.misfirePolicy">
                  <el-radio-button v-for="item in jobMisfireList" :key="item.value" :label="item.value">{{ item.label
                    }}</el-radio-button>
                </el-radio-group>
              </el-form-item>
            </el-col>
            ...
            <el-col :span="12">
              <el-form-item label="状态">
                <el-radio-group v-model="formData.status">
                  <el-radio v-for="dict in jobStatusList" :key="dict.value" :label="dict.value">{{ dict.label
                    }}</el-radio>
                </el-radio-group>
              </el-form-item>
            </el-col>
      ...
      <!-- 任务日志详细 -->
      <el-dialog title="任务详细" v-model="openView" width="700px" append-to-body @close="closeDialog">
          ...
          <el-row>
            ...
            <el-col :span="12">
              <span v-for="item in jobDictList" :key="item.value">
                <el-form-item label="任务分组：" v-if="item.value === formData.jobGroup">
                  {{ item.label }}
                </el-form-item>
              </span>
